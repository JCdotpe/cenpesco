...
$extensiones=array("flv","swf","avi","mpeg","mpg","mp3","pptx","PPTX");
// path adonde la voy a guardar, en este caso mi_ubicacion_actual/imagenes
	...
	$opcion=0;
	  //guardamos datos
$sql="Insert into video values ('','".$titulo."','".$fecha."','".$path."','".$id_modulo."','".$descripcion."')";
$result = mysql_query($sql);
 // $result = mysql_query($sql);
  	...
  	if (@$admitido){
       ...
       $path.='video'.$lastid.'_2013.'.$var[1]; 
        echo $path.'/ '.$lastid;
    	$sql="UPDATE video SET path='".$path."' WHERE id_archivo='".$lastid."'";
		$result = mysql_query($sql);	
...
if($ide==4 and $id_archivo!=NULL ) {
 $query="DELETE  FROM video WHERE id_archivo='".($id_archivo)."'";
...
$result = mysql_query("SELECT * FROM  video WHERE id_archivo='".$id_archivo."'");
...
{
  $sql="UPDATE video SET titulo='$titulo',fecha='$fecha',descripcion='$descripcion' WHERE id_archivo='".$codigo_archivo."'"; 
  mysql_query($sql);	
     $tipo=0;
//---------------edit archi

if (isset($_FILES['archivo']['name'])){ // si estoy subiendo el archivo o es la primera carga de la
$extensiones=array("flv","swf","avi","mpeg","mpg","mp3");
// path adonde la voy a guardar, en este caso mi_ubicacion_actual/imagenes
	$nombre1=$_FILES['archivo']['name'];
	$tamanio=$_FILES['archivo']['size'];
	$tipo=$_FILES['archivo']['type'];
	...
	if (@$admitido){
     // echo $pather.'==';
	  ...
	  {	 
		...
		$path='videos/video'.$codigo_archivo.'.'.$var[1]; 

    	$sql="Update video set path='$path' WHERE id_archivo='".($codigo_archivo)."'";
		$result = mysql_query($sql);	
...
<div class=panel>
  <p>&nbsp;</p>
  <p><span class="titulo2">Lista de Videos y Animaciones</span><br />
  </p>
  <hr />
  <form id="form1" name="form1" method="post" action="edit_recurso.php"  enctype="multipart/form-data">
  <!-- full-name input-->
  <table width="700" border="0">
    <tr>
      <td align="left" valign="top"><label class="texto2">Escriba el título del video o animaci&oacute;n que se va apublicar</label></td>
    </tr>
    <tr>
...
$mysql="SELECT * FROM video order by fecha DESC";  
$r=mysql_query($mysql) or die ("No se puede hacer consulta");
$c=0;
while ($row = mysql_fetch_array($r))
{
    ...
    ?>
        ...
        <tr bgcolor="<?php echo $color;  ?>" >
              ...
              <td width="36" align="center"><a href="edit_recurso.php?ide=3&amp;id_archivo=<?php echo $row[0];  ?>&amp;banderin=1"><img src="fondos/editar.jpg" border="0" width="28px" height="28px"></a></td>
              <td width="39" align="center"><a href="edit_recurso.php?ide=4&amp;id_archivo=<?php echo $row[0];  ?>&valor0=<?php echo $row[3]; ?>"  onClick="return pregunta()"><img src="fondos/eliminar.jpg" border="0" width="28px" height="28px"></a></td>
          </tr>
          <?php } ?>
     </table>
