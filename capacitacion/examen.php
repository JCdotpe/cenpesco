...
<div class=panel>
  <p><span class="titulo2">Lista de ex&aacute;menes pendientes</span><br />
  </p>
<hr />
<?php
         $f_actual=date("Y-m-d");
                            ...
                            if($fecha==date("Y-m-d"))
                              {
                                $fecha="hoy";
								 ...
								 $hh_aa=date($row[4].':00');
								 $hh_aa= date("H:i:s", strtotime("$hh_aa +1 minutes"));
								 
                                 if($h_actual<$hh_aa)
                                   {
                                    $c=$c+1;
                                  echo  '<span class="subtitulo" ><a href="responder.php?id_examen='.$row[0].'&anio='.$anio.'&mes='.$mes.'&dia='.$dia.'&hora='.$hora.'&minuto='.$minuto.'&titulo='.$titulo.'" class="subtitulo" ><b>'.$c.'. Evaluaci&oacute;n pendiente : '.utf8_decode($row[1]).', '.$fecha.' a las '.$row[3].' hrs y culmina a las '.$row[4].' hrs.</b></span><br/><br/>';  
                                   }
                              }
  ...
  if($c==0) { echo utf8_encode('<br/><span class="subtitulo" style="margin-top:18px"><b> No hay examen pendiente para hoy</b></span>'); }
