    ...
    <div id="ap-sidebar" class="span2">
 	...
 	<div class="span10" id="ap-content">
	    		...
	    		echo form_open("/tabulados/export");
					...
					//	
						...
						foreach($tables->result() as $reg){
							...
							$tot = $reg->TOTAL;
							$serie_1[] = round(($a = $reg->PRODUCE)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_2[] = round(($b = $reg->FONDEPES)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_3[] = round(($c = $reg->IIAP)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_4[] = round(($d = $reg->MINAM)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_5[] = round(($e = $reg->SANIPES)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_6[] = round(($f = $reg->IMARPE)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_7[] = round(($g = $reg->DIREPRO)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_8[] = round(($h = $reg->G_REGIONAL)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_9[] = round(($i = $reg->ONG)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_10[] = round(($j   = $reg->OTRO)*100/( ($tot>0) ? $tot : 1 ),2);
							$serie_11[] = round(($k   = $reg->NEP)*100/( ($tot>0) ? $tot : 1 ),2);			
		...
		<?php 
			...
			$series = array(
							array("name" => 'PRODUCE'		,"data" => $serie_1),
							array("name" => 'FONDEPES'		,"data" => $serie_2),
							array("name" => 'IIAP'			,"data" => $serie_3),
							array("name" => 'MINAM'			,"data" => $serie_4),
							array("name" => 'SANIPES'		,"data" => $serie_5),
							array("name" => 'IMARPE'		,"data" => $serie_6),
							array("name" => 'DIREPRO'		,"data" => $serie_7),
							array("name" => 'G. Regional'	,"data" => $serie_8),
