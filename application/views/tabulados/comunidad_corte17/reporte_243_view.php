    ...
    <div id="ap-sidebar" class="span2">
 	...
 	<div class="span10" id="ap-content">
	    		...
	    		echo form_open("/tabulados/export");
					...
					echo '<tbody>';
						...
						foreach($tables->result() as $reg){
							...
							$tot = $reg->TOTAL;
							$serie_1[] = round(($a = $reg->RADIOFONIA)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_2[] = round(($b = $reg->T_FIJA)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_3[] = round(($c = $reg->T_CELULAR)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_4[] = round(($d = $reg->INTERNET)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_5[] = round(($e = $reg->NINGUNO)*100/( ($tot>0) ? $tot : 1 ),2 );
							$serie_6[] = round(($f = $reg->NEP)*100/( ($tot>0) ? $tot : 1 ),2 );
		...
		<?php 
			...
			$series = array(
							array("name" => 'Radiofonía'	,"data" => $serie_1),
							array("name" => 'T. fija'		,"data" => $serie_2),
							array("name" => 'T. celular'	,"data" => $serie_3),
							array("name" => 'Internet'		,"data" => $serie_4),
							array("name" => 'Ninguno'		,"data" => $serie_5),
