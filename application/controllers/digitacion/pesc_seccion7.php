...
class Pesc_seccion7 extends CI_Controller {
	...
	public function index()
	{
		...
		if($is_ajax){
			...
			$fields = $this->pescador_model->get_fields('pesc_seccion7');
			//id
			...
			// if($this->pescador_model->insert_pesc_seccion('pesc_seccion7',$c_data) > 0){
			// 	$flag = 1;
			// 	$msg = 'Se ha registrado satisfactoriamente la Seccion VII';
			// }
			if ($this->pescador_model->consulta_in_seccion($id,'pesc_seccion7')->num_rows() == 0) {
				...
				// actualiza
					if($this->pescador_model->update_pesc_seccion('pesc_seccion7',$c_data,$id) > 0){
						$flag = 1;
