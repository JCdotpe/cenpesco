...
class Comunidad_seccion6 extends CI_Controller {
	...
	public function index()
	{
		...
		if($is_ajax){
					...
					$fields = $this->comunidad_model->get_fields('comunidad_seccion6');
			...
			if ($this->comunidad_model->consulta_in_seccion($id,'comunidad_seccion6')->num_rows() == 0) {
				// inserta nuevo registro
					...
					if($this->comunidad_model->insert_comunidad_seccion('comunidad_seccion6',$c_data) > 0){
						$flag = 1;
				...
				// actualiza
					...
					$flag = 0;
					$msg = 'Error inesperado, por favor intentalo nuevamente';
					if($this->comunidad_model->update_comunidad_seccion('comunidad_seccion6',$c_data,$id) > 0){
						$flag = 1;
