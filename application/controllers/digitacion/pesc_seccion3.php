...
class Pesc_seccion3 extends CI_Controller {
	...
	public function index()
	{
		...
		if($is_ajax){
			...
			$fields = $this->pescador_model->get_fields('pesc_seccion3');
			//id
			...
			// if($this->pescador_model->insert_pesc_seccion('pesc_seccion3',$c_data) > 0){
			// 	$flag = 1;
			// 	$msg = 'Se ha registrado satisfactoriamente la Seccion III';
			// }
			if ($this->pescador_model->consulta_in_seccion($id,'pesc_seccion3')->num_rows() == 0) {
				...
				$c_data['modified'] = date('Y-m-d H:i:s');
					if($this->pescador_model->update_pesc_seccion('pesc_seccion3',$c_data,$id) > 0){
						$flag = 1;
						$msg = 'Se ha modificado satisfactoriamente la Seccion III';
					}
