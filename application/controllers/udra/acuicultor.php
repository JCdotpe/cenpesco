...
class Acuicultor extends CI_Controller {
	...
	public function index()
	{
	    ...
	    if ($this->form_validation->run() === TRUE) {
			...
			if ($this->tank_auth->get_ubigeo()<99) {//VALIDA USUARIO NO PILOTO NO INSERTE

				
				if($this->udra_acuicultor_model->get_centro_poblado($dep,$prov,$dist,$ccpp) > 0){//VALIDA si el CCPP ya fue registrado
