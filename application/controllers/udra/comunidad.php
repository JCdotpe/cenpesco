...
class Comunidad extends CI_Controller {
	...
	public function index()
	{
			...
			$data['tables'] = $this->udra_comunidad_model->get_acuicultores_by_sede($this->tank_auth->get_ubigeo(), $odeis); 
			$data['main_content'] = 'udra/comunidad_view';
			$data['option'] = 3;
			$data['error'] = 0;
	    ...
	    if ($this->form_validation->run() === TRUE) {
			...
			if ($this->tank_auth->get_ubigeo()<99) {
				...
				if($this->udra_comunidad_model->get_centro_poblado($dep,$prov,$dist,$ccpp) > 0){//VALIDA si el CCPP ya fue registrado
					$this->session->set_flashdata('msgbox',3);
					redirect('/udra/comunidad');
				}else{
					...
					if ($od->num_rows() == 1){
		    			...
		    			$afectados = $this->udra_comunidad_model->insert_registro($registros);
				...
				if ( $this->udra_comunidad_model->get_centro_poblado($dep,$prov,$dist,$ccpp) > 0 ) {
					$set =  array(
						'FORMULARIOS' 	=> $this->input->post('formularios'),
						'USUARIO_M'		=> $this->tank_auth->get_user_id(),
						'FECHA_M'		=> date('y-m-d H:i:s',now()));					
					$modificados = $this->udra_comunidad_model->update_nform($dep,$prov,$dist,$ccpp, $set);
					if ($modificados == 1) {
