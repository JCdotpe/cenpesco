...
class Capacitar extends CI_Controller {
	...
	public function index(){
			...
			$data['main_content'] = 'convocatoria/acapacitar_view';
	...
	public function get_regs()
	{
		...
		if($is_ajax){
			$data['datos'] = $this->regs_model->get_regs(2)->result();
			$this->load->view('backend/json/json_view', $data);	
		}else{
			show_404();;
		}
	...
	public function get_regs_by_dist()
	{
		...
		if($is_ajax){
			...
			$data['datos'] = $this->regs_model->get_regs_by_dep($cod_dep,2)->result();
			$this->load->view('backend/json/json_view', $data);	
		}else{
			show_404();;
		}
	...
	public function discapacitar_pers()
	{
		$inscrito = 2;
		$acapacitar = 1;
		$is_ajax = $this->input->post('ajax');
		...
		if($is_ajax){
			...
			$datos['msg'] = 'Se ha realizado la deseleccion con éxito.';
			$datos['flag'] = $flag;
