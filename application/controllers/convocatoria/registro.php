...
class Registro extends CI_Controller {
	...
	public function index($c = null)
	{
		$data['nav'] = TRUE;
		$data['main_content'] = 'convocatoria/inscripcion_view';
		$data['otros'] = FALSE; 
		$data['coordinador'] = FALSE; 
		$data['empadronador'] = FALSE; 
		$data['asistente'] = FALSE; 
		
		if (isset($c) && !is_null($c) && $c == 'cd'){
			$data['asistente'] = TRUE; 
			$data['coordinador'] = TRUE; 
		}else{
			$data['empadronador'] = TRUE; 
			$data['asistente'] = TRUE; 
			$data['coordinador'] = TRUE; 			
		}
	...
	public function inscripcion($c = null)
	{
		// if(isset($c) && !is_null($c) && $c == 'cd')
		// $data['cargos_c']= array(3 => 'ASISTENTE DE COORDINADOR DEPARTAMENTAL', 4 => 'COORDINADOR DEPARTAMENTAL');
		// else
		// $data['cargos_c']= array( 1 => 'EMPADRONADOR');
		$data['cargos_c']= array(1 => 'EMPADRONADOR', 3 => 'ASISTENTE DE COORDINADOR DEPARTAMENTAL', 4 => 'COORDINADOR DEPARTAMENTAL');
		$data['nav'] = TRUE;
		$data['mensaje']=null;
		$data['title'] = 'Inicio';
		...
		//$data['paises']=$this->paises_model->select_pais();
		...
		$datos = null;
		$flag = 0;
		$msg = '<h3>Gracias por registrarse. <a href="' . site_url('convocatoria/consulta') . '">Continuar</a><h3>';
		//

		$this->form_validation->set_rules('departamento','Departamento','required|alpha_numeric');
		$this->form_validation->set_rules('cod_odei','ODEI','required|alpha_numeric');
		// $this->form_validation->set_rules('provincia','Provincia','required|alpha_numeric');
		// $this->form_validation->set_rules('distrito','Distrito','required|alpha_numeric');
		...
		//$this->form_validation->set_rules('paises','Pais','required|alpha_numeric');
        ...
        if ($this->form_validation->run() === TRUE)
        {
			...
			$c_data = array(
					...
					'cod_odei'=> $this->input->post('cod_odei'),
					'nom_odei'=> $this->input->post('nom_odei'),
			...
			if(!$flag){
				$msg = '<h3>Error inesperado, intentelo nuevamente.</h3>';
			}else{
				$flag = 1;
        	...
        	if(!$is_ajax){
        		...
        		//Validación de errores SERVIDOR
        		$datos['flag'] = $flag;
				$datos['datos'] = $this->form_validation->error_array();
       			$data['datos'] = $datos;		
       			$this->load->view('backend/json/json_view', $data);						   
        	}
        }
	...
	public function check_dni()
	{
		...
		if($is_ajax){
			$flag = 1;
			if($this->regs_model->check_dni($code) > 0)
				$flag = 0;
			$datos['flag'] = $flag;
			$data['datos'] = $datos;
			$this->load->view('backend/json/json_view', $data);		
