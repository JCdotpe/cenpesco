...
echo '<select name="S4_1_DI_COD" id="S4_1_DI_COD" onchange="'."carga_rCepo('".$coddis."')".'" class="letra_frm"> <option value="">Seleccionar</option>';
				   ...
				   while ($row = mysql_fetch_row($result)){
                        ...
                        if ($S2_9_DI_COD==substr($row[3],4,2)) {  echo '<option value="'.substr($row[3],4,2).'" selected="selected">'.utf8_decode($row[6]).'</option>'; } else
