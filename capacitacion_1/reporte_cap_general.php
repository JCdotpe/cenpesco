...
$col=0;
$result = mysql_query("SELECT DISTINCT(id_examen) FROM  cap_calificacion WHERE id_modulo='".$id_modulo."'");
...
if ($row = mysql_fetch_row($result))
 {
  ...
  if($opcion==1)
    {
     $nombres=$row[8].' '.$row[9].' '.$row[10].' '.$row[11];
    }
  if($opcion==2)
    {
     $nombres=$row[3];
...
{
$nombre='';
$result = mysql_query("SELECT nota FROM  cap_calificacion WHERE id_modulo='".$id_modulo."' AND dni='".$dni."' ORDER BY id_examen ASC");
while ($row = mysql_fetch_row($result))
 {
...
<?php
    ...
    $result = mysql_query("SELECT DISTINCT(id_examen) FROM  cap_calificacion WHERE id_modulo='".$id_modulo."' ORDER BY id_examen ASC");
     while ($row = mysql_fetch_row($result))
    {
?>
...
if($dep==NULL)
{
$result = mysql_query ("SELECT dni,SUM(nota*peso/100),id_examen FROM  cap_calificacion WHERE id_modulo='".$id_modulo."' GROUP BY dni ORDER BY SUM(nota*peso/100) DESC");
}

if($dep!=NULL)
{
 
 $mysql="SELECT c.dni,SUM(c.nota*c.peso/100),r.cod_dep FROM cap_calificacion AS c INNER JOIN regs AS r on(c.id=r.id) WHERE  c.id_modulo='".$id_modulo."' GROUP BY c.dni ORDER BY r.cod_dep,SUM(c.nota*c.peso/100) DESC";  
