...
//--------------------------------------------------------------------------------
$result = mysql_query("SELECT * FROM regs  WHERE cod_dep='".$cod_dep."'");
...
if($dep!=NULL)
{
 ...
 $mysql="SELECT c.dni,SUM(c.nota*c.peso),r.cod_dep FROM cap_calificacion AS c INNER JOIN regs AS r on(c.id=r.id) WHERE r.cod_dep='".$cod_dep."' GROUP BY c.dni ORDER BY r.cod_dep,SUM(c.nota*c.peso) DESC";  
...
<div>
<br/>
<strong>|<a href="report2_excel.php?cod_dep=<?php echo $cod_dep; ?>&dep=1&nb=<?php echo $nb; ?>"> Reporte Excel</a> |</strong><br/>
</div>
