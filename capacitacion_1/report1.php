...
if($opcion==4)
    {
	 ...
	 if($row[28]==3)
	 {$nombres='ASISTENTE DE COORDINADOR';
	 }
	 
	if($row[28]==4)
	 {$nombres='COORDINADOR DEPARTAMENTAL';
	 }
  ...
  <tr>
   <td width="40" align="left"><strong> N° </strong></td>
    <td width="220" align="left"><strong>Apellidos y Nombres</strong></td>
        <td width="180" align="center"><strong> DNI</strong></td>
    <td width="180" align="center"><strong> ODEI</strong></td>
        <td width="180" align="center"><strong> Cargo</strong></td>
...
<?php
   ...
   // $result = mysql_query("SELECT DISTINCT(id_examen) FROM  cap_calificacion ORDER BY id_examen ASC");
    ...
    {
?>
    <td width="180" align="center"><strong><?php echo ($row["titulo"]); ?></strong></td>
<?php
     }
?>
...
if($dep==NULL)
{
$result = mysql_query ("SELECT dni,SUM(nota*peso),id_examen FROM  cap_calificacion   GROUP BY dni ORDER BY SUM(nota*peso) DESC");
}
...
if($dep!=NULL)
{
 ...
 $mysql="SELECT c.dni,SUM(c.nota*c.peso),r.cod_dep FROM cap_calificacion AS c INNER JOIN regs AS r on(c.id=r.id) GROUP BY c.dni ORDER BY r.cod_dep,SUM(c.nota*c.peso) DESC";  
...
?>
  ...
  <td align="center"><?php echo utf8_decode(regs($row[0],3)); ?></td>
  <td align="center"><?php echo utf8_decode(regs($row[0],2)); ?></td>

     <td align="center"><?php echo utf8_decode(regs($row[0],4)); ?></td>
